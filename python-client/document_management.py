...
def create_items(container):
    ...
    sales_order = get_sales_order(f"{uuid.uuid4()}")
    container.create_item(body=sales_order)
    print(f"Item with id {sales_order['id']} created")
    ...
    sales_order2 = get_sales_order_v2(f"{uuid.uuid4()}")
...
def get_sales_order(item_id):
    order1 = {'id': item_id,
              'account_number': f'Account{random.randint(1, 100)}',
              'purchase_order_number': f'PO{random.randint(1, 18009186470)}',
